...
function getSearchQuery(intentRequest, callback) {
  ...
  if (intentRequest.currentIntent.name === "GetSearchQuery") {
    searchImages(AUTH_KEY, searchQuery).then(res => {
      var params = {
        TableName: "hello-world", //table name in DynamoDB
        Item: {
          date: 123456789, //unique id, can be anything, in this instance, just an arbitrary number
          message: res.hits[0] //pushes the first item in the array of image objects returned from API query
        }
      };
      callback(
        close(
